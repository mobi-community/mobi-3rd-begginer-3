...
const Pagination = () => {
  const { getPaginationParam, updateCurrentPage,checkIsPrevButtonVisible ,checkIsNextButtonVisible } = usePagination()
  ...
  return (
    <div>
      {
        isPrevButtonVisible && (
          <button onClick={()=>updateCurrentPage(startPage-1)}>
            이전
          </button>
        )
      }
      ...
      {
        isNextButtonVisible && (
          <button onClick={()=>updateCurrentPage(endPage+1)}>
            다음
          </button>
        )
      }
